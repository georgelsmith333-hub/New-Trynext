...
export class ObjectStorageService {
  ...
  /** Generic private-object write for server-generated bytes (mockup render
   *  jobs: artwork copies, rendered previews/finals) — same storage path as
   *  saveMockupImage but takes raw bytes instead of a data URL. */
  async saveBuffer(buf: Buffer, ext: string, prefix = "mockup-job"): Promise<string> {
    const safeExt = ["png", "jpg", "jpeg", "webp"].includes(ext) ? ext : "bin";
    const { randomUUID: uuid } = await import("crypto");
    const entityId = `${prefix}-${uuid()}.${safeExt}`;
    ...
    if (BACKEND === "r2" || BACKEND === "s3") {
      ...
      await client.send(
        new PutObjectCommand({
          ...
          Key: `uploads/${entityId}`,
          Body: buf,
          ContentType: safeExt === "bin" ? "application/octet-stream" : `image/${safeExt}`,
        })
      );
    } else {
      const destPath = path.join(localUploadsDir(), entityId);
